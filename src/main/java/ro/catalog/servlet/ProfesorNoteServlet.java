...
@WebServlet("/app/profesor/note")
public class ProfesorNoteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (materii.isEmpty()) {
            ...
            req.setAttribute("note", java.util.Collections.emptyList());
            req.setAttribute("studenti", java.util.Collections.emptyList());
            req.getRequestDispatcher("/WEB-INF/profesor-note.jsp").forward(req, resp);
            return;
        }
        ...
        List<ProfesorNotaView> note = notaDAO.listNoteForProfesor(u.getId(), materieIdSel, clasaId, q, from, to);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("add".equalsIgnoreCase(action)) {
            ...
            Integer valoare = parseIntOrNull(req.getParameter("valoare"));
            LocalDate dataNota = parseDateOrNull(req.getParameter("data"));
            if (studentId == null || valoare == null || dataNota == null) {
                redirectBack(req, resp, materieId, clasaId, q, from, to, "Completează student, notă și dată.", true);
                return;
            }
            if (valoare < 1 || valoare > 10) {
                redirectBack(req, resp, materieId, clasaId, q, from, to, "Nota trebuie să fie între 1 și 10.", true);
                return;
            }

            notaDAO.insertNota(studentId, materieId, valoare, dataNota);
            logDAO.log(u.getEmail(), u.getRol(), "ADD_NOTA student_id=" + studentId + ", materie_id=" + materieId + ", valoare=" + valoare);
            redirectBack(req, resp, materieId, clasaId, q, from, to, "Notă adăugată cu succes.", false);
            return;
        }

        if ("delete".equalsIgnoreCase(action)) {
            Integer notaId = parseIntOrNull(req.getParameter("notaId"));
            if (notaId == null) {
                redirectBack(req, resp, materieId, clasaId, q, from, to, "NotaId lipsă.", true);
                return;
            }
            boolean ok = notaDAO.deleteIfProfesorOwns(notaId, u.getId());
            if (ok) {
                logDAO.log(u.getEmail(), u.getRol(), "DELETE_NOTA id=" + notaId);
                redirectBack(req, resp, materieId, clasaId, q, from, to, "Notă ștearsă.", false);
            } else {
                redirectBack(req, resp, materieId, clasaId, q, from, to, "Nu poți șterge această notă.", true);
            }
            return;
        }
    ...
    private static void redirectBack(HttpServletRequest req, HttpServletResponse resp,
        ...
        url.append(req.getContextPath()).append("/app/profesor/note");
