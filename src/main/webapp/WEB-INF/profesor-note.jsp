...
<div class="wrap">
  ...
  <div class="head">
    <div>
      ...
      <div class="subtitle">Filtrare pe clasă / nume + adăugare note doar la materia ta.</div>
    </div>
  </div>
  ...
  <!-- FILTRE (GET) -->
  <div class="cardX">
    <form method="get" action="${pageContext.request.contextPath}/app/profesor/note">
      <div class="filters">
        <div class="f">
          <label>Materie</label>
        ...
        <div class="actions" style="display:flex; gap:12px; justify-content:flex-end; flex-wrap:wrap;">
          ...
          <a class="btnS secondary" href="${pageContext.request.contextPath}/app/profesor/note">Reset</a>

          <a class="btnS secondary"
             href="${pageContext.request.contextPath}/app/profesor/export?entity=note&fmt=pdf&materieId=${materieIdSel}&clasaId=${clasaIdSel}&q=${q}&from=${from}&to=${to}">
            Export PDF
          </a>

          <a class="btnS secondary"
             href="${pageContext.request.contextPath}/app/profesor/export?entity=note&fmt=xls&materieId=${materieIdSel}&clasaId=${clasaIdSel}&q=${q}&from=${from}&to=${to}">
            Export Excel
          </a>
        </div>
      </div>
  ...
  <!-- ADAUGA NOTA (POST) -->
  <div class="cardX">
    <div style="font-weight:900; margin-bottom:10px;">➕ Adaugă notă</div>
    <form method="post" action="${pageContext.request.contextPath}/app/profesor/note">
      ...
      <div class="filters" style="grid-template-columns: 2fr 1fr 1fr auto;">
        ...
        <div class="f">
          ...
          <input type="number" name="valoare" min="1" max="10" required/>
        </div>
  ...
  <div class="cardX">
    <div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap;">
      <div style="font-weight:900;">📋 Note găsite</div>
      <div class="pill2">${fn:length(note)} înregistrări</div>
    </div>

    <div style="height:10px"></div>

    <c:if test="${empty note}">
      <div style="color:var(--muted); font-size:13px;">Nu există note pentru filtrele curente.</div>
    </c:if>
    ...
    <c:if test="${not empty note}">
      <table>
        ...
        <tbody>
        <c:forEach var="n" items="${note}">
          <tr>
            <td><c:out value="${n.data}"/></td>
            <td><b><c:out value="${n.studentNume}"/></b></td>
            <td><c:out value="${n.clasaNume}"/></td>
            <td><c:out value="${n.materie}"/></td>
            <td>
              <span class="pill2 <c:if test='${n.valoare >= 5}'>good</c:if><c:if test='${n.valoare < 5}'>bad</c:if>">${n.valoare}</span>
            </td>
            <td>
              <form method="post" action="${pageContext.request.contextPath}/app/profesor/note" style="margin:0">
                ...
                <input type="hidden" name="from" value="${from}"/>
                <input type="hidden" name="to" value="${to}"/>
                <button type="submit" class="btnS secondary danger" onclick="return confirm('Ștergi nota?');">Șterge</button>
              </form>
            </td>
          </tr>
        </c:forEach>
